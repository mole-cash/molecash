...
The Mole Core daemon can be configured to act as a trusted "border
router", implementing the mole wire protocol and relay, making
...
The ZeroMQ feature in Mole Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building moled:
...
For instance:
    ...
    $ moled -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/moled.tx.raw \
...
These options can also be provided in mole.conf.
...
From the perspective of moled, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into moled directly. Furthermore, no information is
...
using. Moled appends an up-counting sequence number to each
