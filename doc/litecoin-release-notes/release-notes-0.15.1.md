Mole Core version *0.15.1* is now available from:

  <https://download.mole.org/mole-0.15.1/>
...
A number of changes to the way Mole Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 tMCASH per kB (0.00001 MCASH). This is to prepare for dropping the minimum transaction fee to 0.1 tMCASH per kB (0.0001 MCASH) in the next release.
