#!/usr/bin/env python3
# Copyright (c) 2024 The Mole Core developers
...
def create_non_hd_wallet(chain, options):    
    ...
    conf_file = os.path.join(data_dir, 'mole.conf')
    ...
    v15_node = TestNode(
        ...
        timewait=60,
        timeout_factor=1.0,
        bitcoind=os.path.join(bin_dir, 'moled'),
        bitcoin_cli=os.path.join(bin_dir, 'mole-cli'),
        version=version,
        coverage_dir=None,
        cwd=options.tmpdir,
        extra_args=["-usehd=0"],
