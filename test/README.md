This directory contains integration tests that test moled and its
...
moled and mole-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the mole utilities, currently only
mole-tx.
...
Before tests can be run locally, Mole Core must be built.  See the [building instructions](/doc#building) for help.
...
The P2P and RPC ports used by the moled nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another moled
process running on the system (perhaps from a previous test which hasn't successfully
killed all its moled nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other moled processes are running.

On linux, the test framework will warn if there is another
moled process running when the tests are started.

If there are zombie moled processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
moled processes running on the system, so should not be used if any non-test
moled processes are being run.**
...
sure moled processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and moled `debug.log`s will all be dumped
...
`test_framework.log` and moled `debug.log`s can be combined into a single
...
call methods that interact with the moled nodes-under-test.

If further introspection of the moled instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/moled.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/moled <pid>
```
