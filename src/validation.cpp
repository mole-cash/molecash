    ...
    for (const CTxInput& txin : tx.GetInputs())
    {
        ...
        if (ptxConflicting) {
            if (!setConflicts.count(ptxConflicting->GetHash()))
            {
                ...
                // Mole: Only support BIP125 RBF when -mempoolreplacement arg is set
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    CAmount nSubsidy = 2100 * COIN;
    // Subsidy is cut in half every 210,000 blocks which will occur approximately every 2.38 years.
    nSubsidy >>= halvings;
    return nSubsidy;
}
