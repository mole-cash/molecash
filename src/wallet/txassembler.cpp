    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional MCASH outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes MCASH and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::MCASH_TO_MCASH) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        // First try to construct a MCASH-to-MCASH transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::MCASH_ONLY;
        ...
        // If MCASH-to-MCASH fails, create a peg-out transaction
