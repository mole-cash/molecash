 ...
 */
static const char* BITCOIN_PID_FILENAME = "moled.pid";
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/mole-project/mole>";
...
bool AppInitMain(const util::Ref& context, NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (fs::exists(config_file_path)) {
        ...
        if (configFile != NULL) {
            std::string strHeader = "# Mole(MCASH) config file:\n"
                                    "rpcuser=username\n"
                                    ...
                                    "addnode=seed1.mole.cash\n"
                                    "addnode=seed2.mole.cash\n"
                                    "\n";
    ...
    if (args.IsArgSet("-datadir") && !fs::path(args.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if mole is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if mole is started while in a temporary directory.\n",
